...
def attention(x: np.ndarray, index, attn_w, attn_b, proj_w, proj_b):
    ...
    if kv_cache[index] is None: #prefill, x=(len,728)
        qkv = x @ attn_w + attn_b
...
def gpt2_forward(w, ids, i) -> np.ndarray:
    x = embedding(w, ids, i)
...
def generate(prompt,token = 10, temperature = 1.0, top_k = 5):
    ...
    for _ in range(token):
        if not prefilled:
            logit = gpt2_forward(W, ids, 0)[-1]
        else:
            logit = gpt2_forward(W, [ids[-1]], len(ids)-1)[-1]
...
kv_cache: list[np.ndarray] = [None for _ in range(12)]
np.random.seed(0)
